...
int __init mx53_clocks_init(unsigned long ckil, unsigned long osc, unsigned long ckih1, unsigned long ckih2)
{
	...
	if(machine_is_ge_imx53_modul()) {
#ifdef CONFIG_MACH_GE_IMX53_MODUL_GHZ
		/*
		 * manfred.schlaegl@ginzinger.com
		 * imx53 on ge_imx53_modul is IEC, but use CEC to reach 1GHz
		 */
		pr_warn("GHZ-Support for ge_imx53_modul is experimental and drives CPU out of specification!\n");
		mx53_set_cpu_part_number(IMX53_CEC);
#else
		/*
		 * manfred.schlaegl@ginzinger.com
		 * imx53 on ge_imx53_modul is IEC
		 */
		mx53_set_cpu_part_number(IMX53_IEC);
#endif
		...
		if (pll1_rate > 1000000000) {
			printk(KERN_INFO "XXXXXXXX CEC_1_2G\n");
			mx53_set_cpu_part_number(IMX53_CEC_1_2G);
		} else if (pll1_rate > 800000000) {
			printk(KERN_INFO "XXXXXXXX CECG\n");
			mx53_set_cpu_part_number(IMX53_CEC);
		} else {
			printk(KERN_INFO "XXXXXXXX AEC\n");
	...
	/*
	 * manfred.schlaegl@ginzinger.com
	 * uart_main_clk-parent is pll3 instead of pll2 on ge_imx53_modul
	 */
	if(machine_is_ge_imx53_modul()) {
