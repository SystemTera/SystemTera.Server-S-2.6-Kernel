...
static int __init mxc_init_uart(void)
{
	 ...
	 * ge_imx53 specific settings
	 */
	if(machine_is_ge_imx53_modul()) {
		for(i=0;i<ARRAY_SIZE(mxc_ports);i++) {
			/* dma on uart seems to be broken -> disable */
			mxc_ports[i].dma_enabled=0;
			/* dma buffer-size (if dma enabled) */
			mxc_ports[i].dma_rxbuf_size=1024;
			
			/* enable hardware-controlled cts handling */
			mxc_ports[i].cts_threshold=16;
		}
		/*
		 * configure uart1 (ttymxc0) in DTE-Mode, to be able to use RI as input
		 * caution RX/TX pads are swapped
		 * all other uarts are configured in DCE-Mode
		 * caution RTS/CTS pads are swapped
		 */
		mxc_ports[0].mode=MODE_DTE;
	}
 ...
 * see serial.h
 */
int mx5_set_mxc_uart_receive_trigger_level(int dev_idx, int rxtl)
{
	if(dev_idx>=ARRAY_SIZE(mxc_ports))
		return -ENODEV;
	if(rxtl<0 || rxtl>32)
		return -EINVAL;
	mxc_ports[dev_idx].rx_threshold=rxtl;
	return 0;
}

int mx5_set_mxc_uart_sender_enable_func(
		int dev_idx, 
		mxc_uart_sender_enable_func sender_enable_func,
		bool half_duplex)
{
	if(dev_idx>=ARRAY_SIZE(mxc_ports))
		return -ENODEV;
	mxc_ports[dev_idx].sender_enable_func=sender_enable_func;
	mxc_ports[dev_idx].half_duplex=half_duplex;
	return 0;
}
