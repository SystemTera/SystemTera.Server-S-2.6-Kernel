...
/*!
 * manfred.schlaegl: callback for sender enable
 * this callback, if registered, will be called:
 * 	if data-transmit is started -> ena=true
 * 	if data-transmit is done -> ena=false
 * board-code is able to realize a read-write direction-switch (rs485_rw) by this
 */
typedef void (*mxc_uart_sender_enable_func)(bool ena);
...
typedef struct {
	...
	/*!
	 * callback for sender enable/disable
 	 * board-code is able to realize a read-write direction-switch (rs485_rw) by this
	 */
	mxc_uart_sender_enable_func sender_enable_func;
	/*!
 	 * if half_duplex is set, the receiver-interrupt will be disabled while send
	 */
	bool half_duplex;
	/*!
	 *  current state of sender/receiver enable 
	 */
	bool sender_enabled;
} uart_mxc_port;
