 ...
 * problem: pwm needs to be enabled/disabled in interrupt-context by pwm-beeper, but
 * 	call of clk_enable/clk_disable is not allowed in interrupt-context.
 * solution: call clk_enable on pwm_request and clk_disable on pwm_free, instead
 *	of in pwm_enable/pwm_disable.
 * theoretically this causes less dynamic power-managment, but has practically no impact.
 * (disable following define, to enable the original handling)
 */
#define PWM_ENABLE_CLKS_PERMANENTLY
...
#ifndef PWM_ENABLE_CLKS_PERMANENTLY
	if (!pwm->clk_enabled) {
...
#ifdef PWM_ENABLE_CLKS_PERMANENTLY
			if (!pwm->clk_enabled) {
