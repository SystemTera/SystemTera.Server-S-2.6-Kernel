...
static int __devinit ge_counter_probe(struct platform_device *pdev)
{
	struct ge_counter_platform_data *pdata = pdev->dev.platform_data;
	...
	for (i = 0; i < pdata->num_counter; i++) {
		...
		set_irq_type(irq, src->irq_type);
		enable_irq_wake(irq);
		ret = request_irq(irq, counter_event, 0, "ge_counter", dest);
		if (ret < 0) {
		...
		dest->counter_attr.attr.mode = 0644;
		...
		dest->elapsed_attr.attr.mode = 0644;
		...
		dest->state_attr.attr.mode = 0444;
		...
		dest->reset_on_read_attr.attr.mode = 0644;
		...
		if (src->interval) {
			...
			dest->interval_attr.attr.mode = 0644;
			...
			dest->interval_count_attr.attr.mode = 0444;
...
static int __devexit ge_counter_remove(struct platform_device *pdev)
{
...
static struct platform_driver ge_counter_driver = {
	...
	.remove        = __devexit_p(ge_counter_remove),
};
