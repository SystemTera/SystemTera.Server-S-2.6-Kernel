...
static int __devinit ge_variables_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_variables; i++) {
		...
		dest->value.attr.attr.mode = 0444;
...
static int __devexit ge_variables_remove(struct platform_device *pdev)
{
...
static struct platform_driver ge_variables_driver = {
	...
	.remove        = __devexit_p(ge_variables_remove),
};
