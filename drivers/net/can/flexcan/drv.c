...
static int flexcan_hw_restart(struct net_device *dev)
{
	...
	flexcan->stat.netif_queue_stopped=0;
#endif
...
static void flexcan_hw_watch(unsigned long data)
{
	...
	if (reg & __MCR_MDIS) {
		if (flexcan_hw_restart(dev)) {
			/* manfred.schlaegl@ginzinger.com: mod_timer uses absolute jiffies instead of relative (HZ/20 = 50ms) */
			mod_timer(&flexcan->timer, jiffies + msecs_to_jiffies(50));
		}
		return;
	}
	ecr = __raw_readl(flexcan->io_base + CAN_HW_REG_ECR);
	if (flexcan->boff_rec) {
#warning "manfred.schlaegl@ginzinger.com: experimental software busoff recovery"
		/* 
		 * manfred.schlaegl@ginzinger.com
		 * do software bus-on recovery; hardware bus-on doesn't work correctly
		 */
#if 0
		/* manfred.schlaegl@ginzinger.com: original by freescale -> doesn't work */
...
#else
		/* manfred.schlaegl@ginzinger.com: try to restart stopped and soft-resetted hw(flexcan_hw_busoff) */
		if(flexcan_hw_restart(dev)) {
			/* retry */
			mod_timer(&flexcan->timer, jiffies + msecs_to_jiffies(50));
			return;
...
#warning "manfred.schlaegl@ginzinger.com: experimental software busoff recovery"
		 ...
		 * manfred.schlaegl@ginzinger.com
		 * do software bus-on recovery; hardware bus-on doesn't work correctly
		 * stop and software-reset -> restart is done in timer-function(flexcan_hw_watch)
		 */
		//printk(KERN_INFO "%s.%s.%i: %s: try software busoff recovery\n",__FILE__,__FUNCTION__,__LINE__,dev->name);
		flexcan_hw_stop(flexcan); /* not strictly necessary */
		reg = __raw_readl(flexcan->io_base + CAN_HW_REG_MCR);
		__raw_writel(reg | __MCR_SOFT_RST, flexcan->io_base + CAN_HW_REG_MCR);
		/* manfred.schlaegl@gmx.at: mod_timer uses absolute jiffies instead of relative (HZ/10 = 100ms) */
		mod_timer(&flexcan->timer, jiffies + msecs_to_jiffies(100));
		return;
	}
	reg = __raw_readl(flexcan->io_base + CAN_HW_REG_MCR);
	__raw_writel(reg | __MCR_SOFT_RST, flexcan->io_base + CAN_HW_REG_MCR);
...
static int flexcan_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	flexcan->stat.netif_queue_stopped=1;
#endif
...
#ifdef CONFIG_CAN_FLEXCAN_STATISTICS
	/* reset statistics */
	memset(&flexcan->stat,0,sizeof(struct flexcan_device_statistics));
#endif
