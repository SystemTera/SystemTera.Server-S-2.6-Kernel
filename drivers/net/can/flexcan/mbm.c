...
{
	int i, iflag1, iflag2, minmb, maxmb;
	struct flexcan_device *flexcan = dev ? netdev_priv(dev) : NULL;
	 ...
	 * acking the interrupts before handling the fifo causes errors,
	 * but interrupts must be acked before mailbox-handling
	 *
	 * solution:
	 * disable following two lines here
	 * __raw_writel(iflag1, flexcan->io_base + CAN_HW_REG_IFLAG1);
	 * __raw_writel(iflag2, flexcan->io_base + CAN_HW_REG_IFLAG2);
	 * ack handled interrupts after fifo-handling (see below)
	 */
	...
	/*
	 * manfred.schlaegl@ginzinger.com
	 * ack handled interrupts after fifo-handling (see above)
	 */
	__raw_writel(iflag1, flexcan->io_base + CAN_HW_REG_IFLAG1);
	__raw_writel(iflag2, flexcan->io_base + CAN_HW_REG_IFLAG2);
	
	for (i = minmb; iflag1 && (i < maxmb); i++) {
		if (iflag1 & (1 << i)) {
...
int flexcan_mbm_xmit(struct flexcan_device *flexcan, struct can_frame *frame)
{
	...
	do {
		...
		if (i == flexcan->xmit_mb) {
			/* sender not ready -> packet-transmission will be stopped -> no lost of messages */
			return -1;
		}
	} while (1);
