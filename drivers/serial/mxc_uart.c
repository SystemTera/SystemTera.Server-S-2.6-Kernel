...
/*
 * this value describes the cylces the isr will repeat
 * interrupt handling without leaving.
 */
#define MXC_ISR_PASS_LIMIT      1 //256	/* lower worst-case per uart irq-latency */
...
/*!
 * This functions calls the sender-enable/disable callback, if registered
 * and disables/enables receiver, if half_duplex-mode is set.
 * (used for rs485 - full/half-duplex-support)
 */
static inline void mxcuart_sender_enable(uart_mxc_port *umxc, bool enable)
{
	volatile unsigned int cr;

	if(umxc->sender_enabled==enable)
		return;

	umxc->sender_enabled=enable;

	if(enable) {
		if(umxc->half_duplex) {
			/* half-duplex-mode: disable receiver */
			cr = readl(umxc->port.membase + MXC_UARTUCR2);
			cr &= ~MXC_UARTUCR2_RXEN;
			writel(cr, umxc->port.membase + MXC_UARTUCR2);
		}
		
		if(umxc->sender_enable_func) {
			/* enable sender */
			umxc->sender_enable_func(true);
		}
	} else {
		if(umxc->sender_enable_func) {
			/* disable sender */
			umxc->sender_enable_func(false);
		}

		if(umxc->half_duplex) {
			/* half-duplex-mode: enable receiver */
			cr = readl(umxc->port.membase + MXC_UARTUCR2);
			cr |= MXC_UARTUCR2_RXEN;
			writel(cr, umxc->port.membase + MXC_UARTUCR2);
		}
...
static irqreturn_t mxcuart_int(int irq, void *dev_id)
{
	...
	do {
		...
		if ((cr1 & MXC_UARTUCR1_TRDYEN) && (sr1 & MXC_UARTUSR1_TRDY)) {
			...
			mxcuart_sender_enable(umxc, true);

			/* Enable Transmit complete intr to reenable RX and disable sender */
		...
		if(sr2 & MXC_UARTUSR2_TXDC) {
			...
			/* disable sender / start receiver */
			mxcuart_sender_enable(umxc, false);
...
static irqreturn_t mxcuart_tx_int(int irq, void *dev_id)
{
	...
	/* enabled sender / disable receiver */	
	mxcuart_sender_enable(umxc, true);
	...
	if(sr2 & MXC_UARTUSR2_TXDC) {
		/* ack TXDC */
		writel(MXC_UARTUSR2_TXDC, umxc->port.membase + MXC_UARTUSR2);
		if (umxc->ir_mode == IRDA && echo_cancel) {
			cr = readl(umxc->port.membase + MXC_UARTUCR2);
			cr |= MXC_UARTUCR2_RXEN;
			writel(cr, umxc->port.membase + MXC_UARTUCR2);
			/* Disable the Transmit complete interrupt bit and sender */
			cr = readl(umxc->port.membase + MXC_UARTUCR4);
			cr &= ~MXC_UARTUCR4_TCEN;
			writel(cr, umxc->port.membase + MXC_UARTUCR4);
		}
...
static int mxcuart_probe(struct platform_device *pdev)
{
	...
	if (mxc_ports[id]->enabled == 1) {
		...
		/* reset sender */
		if(mxc_ports[id]->sender_enable_func) {
			mxc_ports[id]->sender_enabled=false;
			mxc_ports[id]->sender_enable_func(false);
		}
	}
	return 0;
}
