...
{									\
	if(master_drv_data->transfer.rx_buf) {				\
		*(type *)master_drv_data->transfer.rx_buf = val;	\
		master_drv_data->transfer.rx_buf += sizeof(type);	\
	}								\
}
...
{									\
	...
	if(master_drv_data->transfer.tx_buf) {				\
		val = *(type *)master_drv_data->transfer.tx_buf;	\
		master_drv_data->transfer.tx_buf += sizeof(type);	\
	}								\
	return val;							\
}
