/* revert
 * linux/sound/soc/codecs/tlv320aic3110.c
 *
 *
 ...
 *           related registers.
 *
 * manfred.schlaegl@ginzinger.com : HACKED - MERGED OF tlv320aic31xx.c(TI-Broken) and tlv320aic3x.c(Mainline)
 * 
 */
#warning "manfred.schlaegl@ginzinger.com: CAUTION: USE THIS DRIVER ONLY FOR EXPERIMENTS! HIGHLY UNDER DEVELOPMENT!"

/*
 * manfred.schlaegl@ginzinger.com
 * This driver was sent to us by TI(tlv320aic31xx.c), was heavily broken and needs a complete
 * redesign based on tlv320aic3x.c
 */
...
static struct i2c_client *tlv320aic3110_client;
struct regulator *audio_regulator;
static struct i2c_board_info tlv320aic3110_hwmon_info = {
...
{
	int i;
//printk(KERN_INFO "0000000000000000000000 mclk=%i rate=%i - ignore mclk\n",mclk,rate);
			     ...
			     struct snd_soc_dai *dai)
{
	...
	//printk (KERN_INFO "###aic31xx_hw_params System Clock %i\n", aic31xx->sysclk);
...
//	DBG("###aic31xx_set_dai_sysclk SysClk %x\n", freq);
	...
	case AIC31XX_FREQ_24000000:
		...
		return 0;
	default:
		printk(KERN_WARNING "Invalid frequency to set DAI system clock -> FORCE TO USING IT (manfred.schlaegl@ginzinger.com)\n");
		aic31xx->sysclk = freq;
		return 0;
	}
 ...
 * aic31xx_resume()
 */
struct snd_soc_codec_device soc_codec_dev_aic31xx = {
	.probe = aic31xx_probe,
 ...
 */
struct snd_soc_dai tlv320aic31xx_dai[] = {
{
	.name = "tlv320aic3110",
	.playback = {
 ...
 * This function is to suspend the AIC31xx driver.
 */
static int aic31xx_suspend(struct platform_device *pdev, pm_message_t state)
{
 ...
 * This function is to resume the AIC31xx driver.
 */
static int aic31xx_resume(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec = socdev->card->codec;
	int i; u8 data[2];
...
/**
 * @struct tlv320aic3110_dai_ops
 *
 * The DAI Operations Structure which contains the call-back function
 * Addresses used by the ALSA Core Layer when operating with this Codec
 * Driver.
 */
static struct snd_soc_dai_ops tlv320aic3110_dai_ops = {
	.hw_params = aic31xx_hw_params,
	.digital_mute = aic31xx_mute,
	.set_sysclk = aic31xx_set_dai_sysclk,
	.set_fmt = aic31xx_set_dai_fmt,
 ...
 * 192k The PCM bit format supported are 16, 20, 24 and 32 bits
 */
struct snd_soc_dai tlv320aic3110_dai = {
	.name = "tlv320aic3110",
	.playback = {
		.stream_name = "Playback",
		.channels_min = 1,
...
static int aic3x_init(struct snd_soc_codec *codec)
{
	int reg,i;
	struct aic31xx_priv *aic31xx = snd_soc_codec_get_drvdata(codec);
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);
...
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);
	...
	codec->name = "tlv320aic3110";
	codec->owner = THIS_MODULE;
	codec->read = aic31xx_read,
	codec->write = aic31xx_write,
	codec->set_bias_level = aic31xx_set_bias_level,
	codec->dai = &tlv320aic3110_dai;
	codec->num_dai = 1;
	codec->reg_cache_size = ARRAY_SIZE(aic31xx_reg);
	codec->reg_cache_size = sizeof(u8);
	codec->reg_cache = kmemdup(aic31xx_reg, sizeof(aic31xx_reg), GFP_KERNEL);
...
static int tlv320aic3110_register(struct snd_soc_codec *codec)
{
	int ret;
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);
...
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);
	aic31xx_codec = codec;
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i name=%s\n",__FILE__,__FUNCTION__,__LINE__,codec->name);
...
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);
	ret = snd_soc_register_dai(&tlv320aic3110_dai);
	if (ret) {
...
static int tlv320aic3110_unregister(struct aic31xx_priv *aic3x)
{
	aic31xx_set_bias_level(&aic3x->codec, SND_SOC_BIAS_OFF);

	snd_soc_unregister_dai(&tlv320aic3110_dai);
 ...
 * around
 */
static int tlv320aic3110_i2c_probe(struct i2c_client *i2c,
			   const struct i2c_device_id *id)
{
	struct snd_soc_codec *codec;
	struct aic31xx_priv *aic3xx;
	struct tlv320aic3110_pdata *pdata = i2c->dev.platform_data;
	int ret, i;

//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);

	aic3xx = kzalloc(sizeof(struct aic31xx_priv), GFP_KERNEL);
	if (aic3xx == NULL) {
...
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);
	...
	codec = &aic3xx->codec;
	codec->dev = &i2c->dev;
	snd_soc_codec_set_drvdata(codec, aic3xx);
	codec->control_data = i2c;
	codec->hw_write = (hw_write_t) i2c_master_send;
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);

	i2c_set_clientdata(i2c, aic3xx);

//printk(KERN_INFO "0000000000000000000000 %s.%s.%i name=%s\n",__FILE__,__FUNCTION__,__LINE__,codec->name);
	return tlv320aic3110_register(codec);
}

static int tlv320aic3110_i2c_remove(struct i2c_client *client)
{
	struct aic31xx_priv *aic3x = i2c_get_clientdata(client);

	return tlv320aic3110_unregister(aic3x);
}

static const struct i2c_device_id tlv320aic3110_i2c_id[] = {
	{ "tlv320aic3110", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, tlv320aic3110_i2c_id);

/* machine i2c codec control layer */
static struct i2c_driver tlv320aic3110_i2c_driver = {
	.driver = {
		.name = "aic31xxx I2C Codec",
		.owner = THIS_MODULE,
	},
	.probe	= tlv320aic3110_i2c_probe,
	.remove = tlv320aic3110_i2c_remove,
	.id_table = tlv320aic3110_i2c_id,
};

static inline void tlv320aic3110_i2c_init(void)
{
	int ret;
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);

	ret = i2c_add_driver(&tlv320aic3110_i2c_driver);
	if (ret)
		printk(KERN_ERR "%s: error regsitering i2c driver, %d\n",
		       __func__, ret);
}

static inline void tlv320aic3110_i2c_exit(void)
{
	i2c_del_driver(&tlv320aic3110_i2c_driver);
}

static int tlv320aic3110_probe(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct tlv320aic3110_setup_data *setup;
	struct snd_soc_codec *codec;
	int ret = 0;

	printk(KERN_ERR "%s: manfred.schlaegl@ginzinger.com: CAUTION: USE THIS DRIVER ONLY FOR EXPERIMENTS! HIGHLY UNDER DEVELOPMENT!\n",__FILE__);
...
static int tlv320aic3110_remove(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec = socdev->card->codec;
...
struct snd_soc_codec_device soc_codec_dev_tlv320aic3110 = {
	.probe = tlv320aic3110_probe,
	.remove = tlv320aic3110_remove,
	.suspend = aic31xx_suspend,
	.resume = aic31xx_resume,
};
EXPORT_SYMBOL_GPL(soc_codec_dev_tlv320aic3110);

static int __init tlv320aic3110_modinit(void)
{
//printk(KERN_INFO "0000000000000000000000 %s.%s.%i\n",__FILE__,__FUNCTION__,__LINE__);

	tlv320aic3110_i2c_init();

	return 0;
}
module_init(tlv320aic3110_modinit);

static void __exit tlv320aic3110_exit(void)
{
	tlv320aic3110_i2c_exit();
}
module_exit(tlv320aic3110_exit);
